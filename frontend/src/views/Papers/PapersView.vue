  ...
  <AdminLayout>
    <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10 space-y-6">
      ...
      <div class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
        ...
        <span>/</span>
        <span class="text-gray-900 dark:text-white">Papeles</span>
      </div>
      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-sm text-gray-500 dark:text-gray-400">
            {{
              viewMode === 'list'
                ? 'Gestionar tipos de papel y sus precios'
                : viewMode === 'form'
                  ? 'Configurar detalles del papel'
                  : `Establecer rangos de precio para ${selectedPaper?.name}`
            }}
          </p>
        </div>
      ...
      >
        ...
        <div v-if="showPricingButton" class="mt-3">
          <button
            @click="goToPricing(lastCreatedPaperId!)"
            ...
            <thead>
              <tr class="border-b border-gray-200 dark:border-gray-700">
                ...
                <th class="px-5 py-3 text-left sm:px-6 w-32">
                  <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Peso (g)</p>
                </th>
                <th class="px-5 py-3 text-left sm:px-6 w-2/5">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              ...
              >
                <td class="px-5 py-4 sm:px-6">
                  <div>
                    ...
                    <p class="text-xs text-gray-400">ID: {{ paper.id }}</p>
                  </div>
                </td>
                <td class="px-5 py-4 sm:px-6">
                  <span
              ...
              <tr v-if="filteredPapers.length === 0">
                <td colspan="4" class="px-5 py-8 text-center text-gray-500 dark:text-gray-400">
                  <p class="text-sm">No se encontraron papeles</p>
      ...
      >
        <form @submit.prevent="savePaper" class="space-y-6 max-w-2xl">
          <!-- Validation Errors -->
          <div
          ...
          <div>
            <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
              Nombre del Papel <span class="text-error-500">*</span>
            </label>
            <input
              v-model="formData.name"
              type="text"
              required
              placeholder="ej., Couché 300g"
              :class="[
          ...
          <div>
            <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
              Peso (gramos) <span class="text-error-500">*</span>
            </label>
            <input
              ...
              @blur="validateFormField('weight')"
            />
            <p v-if="formFieldErrors.weight" class="mt-1 text-xs text-error-500">
              {{ formFieldErrors.weight }}
            </p>
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              rows="3"
              placeholder="Ingresa descripción del papel..."
          ...
          <div class="flex items-center gap-3 pt-4">
            ...
            >
              ...
              <span>{{ editingPaper ? 'Actualizar Papel' : 'Crear Papel' }}</span>
            </button>
            <button
              type="button"
              @click="goToList"
      ...
      <!-- Pricing View -->
      <div
        v-if="viewMode === 'pricing' && selectedPaper"
        class="rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]"
      >
        <div class="mb-6 flex items-center justify-between">
          <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400">
              {{ selectedPaper.weight }}g • {{ selectedPaper.description || 'Sin descripción' }}
            </p>
          </div>
          <button
            @click="goToForm(selectedPaper)"
          ...
          >
            ...
            Editar Detalles del Papel
          </button>
        </div>

        <PricingManager
          target-type="paper"
          :target-id="selectedPaper.id"
      ...
      >
        <div class="w-full max-w-md rounded-xl bg-white p-6 shadow-xl dark:bg-gray-800">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Eliminar Papel</h3>
          <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
            ¿Estás seguro de que deseas eliminar <strong>{{ paperToDelete?.name }}</strong
...
const lastCreatedPaperId = ref<number | null>(null)

// Form state
const editingPaper = ref<Paper | null>(null)
const formData = ref<CreatePaperRequest>({
  name: '',
  weight: 0,
  description: '',
})
...
const formErrors = computed(() => {
  ...
  if (!trimmedName) {
    errors.push('El nombre del papel es requerido')
  } else if (trimmedName.length < 2) {
    errors.push('El nombre del papel debe tener al menos 2 caracteres')
  }
...
const validateFormField = (field: string) => {
  ...
  if (field === 'name') {
    const trimmedName = formData.value.name?.trim()
    if (!trimmedName) {
      formFieldErrors.value.name = 'El nombre del papel es requerido'
    } else if (trimmedName.length < 2) {
  ...
  if (field === 'weight') {
    if (!formData.value.weight || formData.value.weight <= 0) {
      formFieldErrors.value.weight = 'El peso debe ser mayor que 0'
    }
  }
}
...
const filteredPapers = computed(() => {
  ...
  return papers.value.filter(
    (paper) =>
      ...
      (paper.description?.toLowerCase().includes(query) ?? false),
  )
})
...
const loadPapers = async () => {
  loading.value = true
  try {
    papers.value = await get<Paper[]>('/papers/')
  } catch (err) {
...
const goToPricing = async (paperId: number) => {
...
const savePaper = async () => {
  ...
  try {
    ...
    if (editingPaper.value) {
      ...
      if (submitData.name !== editingPaper.value.name) updateData.name = submitData.name
      if (submitData.weight !== editingPaper.value.weight) updateData.weight = submitData.weight
      ...
      // Create new
      const newPaper = await post<Paper>('/papers/', submitData)
      successMessage.value = '¡Papel creado exitosamente!'
      showPricingButton.value = true
      lastCreatedPaperId.value = newPaper.id
      await loadPapers()
      resetForm()
    }
  } catch (err: any) {
    console.error('Error saving paper:', err)
    successMessage.value = err.message || 'Error al guardar el papel. Por favor intenta de nuevo.'
  } finally {
    saving.value = false
  }
}
...
const deletePaper = async () => {
  ...
  deleting.value = true
  try {
    await del(`/papers/${paperToDelete.value.id}/`)
    showDeleteModal.value = false
    paperToDelete.value = null
    successMessage.value = '¡Papel eliminado exitosamente!'
    await loadPapers()
  } catch (err) {
